...
var BomItemPurchasing = React.createClass({displayName: "BomItemPurchasing",
  ...
  render: function() {
    ...
    mfgs = [];
    mfgField = bom.getItemFieldForField(item.id || item.cid, FieldConstants.MFG);
    mpnField = bom.getItemFieldForField(item.id || item.cid, FieldConstants.MPN);

    if (mfgField || mpnField) {
      mfgs.push({
        "key": 1,
        "mfg": mfgField ? mfgField.value : undefined,
        "mpn": mpnField ? mpnField.value : undefined
      });
    }
    ...
    suppliers = [];
    supField = bom.getItemFieldForField(item.id || item.cid, FieldConstants.SUPPLIER);
    spnField = bom.getItemFieldForField(item.id || item.cid, FieldConstants.SPN);
    priceField = bom.getItemFieldForField(item.id || item.cid, FieldConstants.PRICE);
    leadTimeField = bom.getItemFieldForField(item.id || item.cid, FieldConstants.LEAD_TIME);
    moqField = bom.getItemFieldForField(item.id || item.cid, FieldConstants.MOQ);

    if (supField || spnField || priceField || leadTimeField || moqField) {
      suppliers.push({
        "key": 1,
        "supplier": supField ? supField.value : undefined,
        "spn": spnField ? spnField.value : undefined,
        "price": priceField ? priceField.value : undefined,
        "leadtime": leadTimeField ? leadTimeField.value : undefined,
        "moq": moqField ? moqField.value : undefined
      });
    }
