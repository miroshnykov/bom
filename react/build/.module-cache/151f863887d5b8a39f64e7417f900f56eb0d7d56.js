...
var Bom = React.createClass({displayName: "Bom",
    ...
    */
    render: function() {
        ...
        var headers = this._getHeadersForFieldSet( this.state.fieldSetId );
        var childBoms;
        var boms;
        var breadcrumbs;
        var fieldSet;
        ...
        if (this.state.isAddColumnModalOpen) {
          addColumnModal = this._getAddColumnModal(bom, headers);
        }
        // edit column modal
        else if (this.state.isEditColumnModalOpen) {
          editColumnModal = this._getEditColumnModal(bom, headers);
        }
        // bom export modal
                        ...
                        React.createElement(ButtonToolbar, {className: "pull-left"}, 
                            ...
                            React.createElement("div", {className: "btn-group btn-group-actions"}, 
                                React.createElement(Button, {
                                    ...
                                    disabled: !this.props.allSelectedBomItems.length}, 
    ...
    _onSaveAddColumn: function(fieldId, typeId, name, index) {
        ...
        if (this.state.fieldSetId === FieldConstants.CUSTOM_FIELDSET) {
            ...
            //get the headers
            headers = this._getHeadersForFieldSet( this.state.fieldSetId );
    ...
    _getHeadersForFieldSet: function(fieldSetId) {
        var bom = this._getBom();
        var headers;
        var fieldSet;
        ...
        if (fieldSetId === FieldConstants.CUSTOM_FIELDSET) {
            ...
            if (fieldSetId === FieldConstants.FULL_FIELDSET) {
                ...
                bom.getAttributes().each(function(attribute) {
                    if (!_.contains(fieldSet.fieldIds, attribute.get("fieldId"))) {
                        headers.push({
                            id: attribute.id || attribute.cid,
                            fieldId: attribute.get("fieldId"),
                            name: attribute.get("name")
