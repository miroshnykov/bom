...
var BomManager = React.createClass({displayName: "BomManager",
    ...
    componentDidMount: function() {
        UserActions.fetch().then(function(user) {
            ...
            if (user.has('companies') && user.get('companies').length > 0) {
                CompanyActions.select(user.get('companies')[0]);
            ...
            BomStore.on("add:items change:items remove:items reset:items", this._onChange);
            BomStore.on("add:attributes change:attributes remove:attributes reset:attributes", this._onChange);
