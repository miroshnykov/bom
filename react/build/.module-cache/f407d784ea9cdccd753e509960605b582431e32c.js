...
var Bom = React.createClass({displayName: "Bom",
    ...
    */
    ...
    _onHideColumn: function(index) {
        ...
        if (this.state.fieldSetId === FieldConstants.CUSTOM_FIELDSET) {
            BomActions.hideColumn(bom.id || bom.cid, columns[index].id || columns[index].cid);
        }
    ...
    _onAddItem: function(event) {
        ...
        newItem = $(this.getDOMNode()).find("table>tbody>tr:last-child");
        if (newItem) {
            console.log(newItem.position());
            vs = $(this.getDOMNode()).getNiceScroll(0);
            console.log(vs);
            vs.doScrollTop(vs.scroll.y + newItem.position().top, 1000);
        }
    },
