            ...
            case ActionConstants.UPDATE_BOM_ITEM:
                (function() {
                    ...
                    if (content === undefined || content === "") {
                        ...
                        //TODO fix this with value collection

                        //get the item's values
                        value = _.find(item.getValues(), function(result) {
                            return result.bomFieldId === attribute.id || result.bomFieldId === attribute.cid;
                        });

                        //remove the value
                        values = item.getValues().map(_.clone);
                        values = _.filter(values, function(result) {
                            return result.bomFieldId !== attribute.id && result.bomFieldId !== attribute.cid;
                        });
                        ...
                        if (value) {
                            ...
                            value = _.clone(value);
                            value.content = action.attributes.content;
