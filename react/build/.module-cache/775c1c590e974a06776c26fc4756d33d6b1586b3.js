        ...
        headers.map(function(result, index) {
          ...
          if (result) {
            return React.createElement(BomItemField, {
              key: result.get("fieldId"), 
              bomId: bom.id || bom.cid, 
              itemId: item.id || item.cid, 
              content: bom.getItemValueContentForField(item.id || item.cid, result.get("fieldId")), 
              attribute: bom.getAttributeForField(result.get("fieldId")), 
              field: allFields.get(result.get("fieldId")), 
              headerName: result.get("name"), 
