...
describe("BomStore", function() {
    ...
    it("parses collection of boms", function() {
        ...
        expect(options.total).toBe(testBoms.length);
    });
    ...
    it("creats a bom", function() {
        ...
        expect(BomStore.get(2).get("bomIds")).toEqual([bom.cid]);

        expect(payload.action.result.bom).toBeDefined();
        expect(payload.action.result.parentBom).toBeDefined();
    });
    ...
    it("adds a blank item", function() {
        ...
        expect(bom.get("items").length).toBe(3);
        expect(bom.get("items")[2].cid).toBeDefined();
    });
    ...
    it("removes items from a bom", function() {
        ...
        expect(bom.get("items").length).toBe(1);
        expect(bom.get("items")[0].id).toEqual(2);
        expect(payload.action.result.boms[0].id).toBe(1);
    ...
    it("updates a bom item for an existing column with a string value", function() {
        ...
        expect(bom.get("items")[1].values.length).toBe(1);
        expect(bom.get("items")[1].values[0].content).toBe("New Value");
        expect(payload.action.result.bom).toBeDefined();
        expect(payload.action.result.field).toBeDefined();
    });
    ...
    it("updates a bom item for an existing column with a number value", function() {
        ...
        expect(bom.get("items")[1].values.length).toBe(1);
        expect(bom.get("items")[1].values[0].content).toBe(1);
        expect(payload.action.result.bom).toBeDefined();
        expect(payload.action.result.field).toBeDefined();
    });
    ...
    it("updates a bom item for an existing column with an empty value", function() {
        ...
        expect(bom.get("items")[1].values.length).toBe(0);
        expect(payload.action.result.bom).toBeDefined();
        expect(payload.action.result.field).toBeDefined();
    });
    ...
    it("updates a bom item for field for which the bom has no column", function() {
        ...
        expect(bom.get("items")[1].values.length).toBe(2);
        expect(bom.get("items")[1].values[1].content).toBe("New Value");

        expect(bom.get("attributes").length).toBe(2);
        expect(bom.get("attributes")[1].name).toBe("New Field");
