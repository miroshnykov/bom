...
var BomModel = ExtendedModel.extend({
    ...
    initializeItems: function() {
        this.listenTo(this.getItems(), "add", function(model, collection, options) {
            this.trigger("add:items", model, collection, options);
            this.validateItem(model);
        });

        this.listenTo(this.getItems(), "change", function(model, options) {
            this.trigger("change:items", model, options);
            this.validateItem(model);
        });

        this.listenTo(this.getItems(), "remove", function(model, collection, options) {
            this.trigger("remove:items", model, collection, options);
            this.validateItem(model);
        });

        //this.listenTo(this.getItems(), "reset", function(collection, options) {
        //     this.trigger("reset:items", collection, options);
        //    this.validateItem();
        //});

        this.listenTo(this.getItems(), "sync", function(collection, resp, options) {
            this.trigger("sync:items", collection, resp, options);
        });

        this.listenTo(this.getItems(), "update", function() {
    ...
    initializeValues: function() {
        ...
        // this.listenTo(this.getItems(), "reset:values", function(collection, options) {
        //     this.validateValue();
        // });

        // this.listenTo(this.getItems(), "sync:values", function(collection, resp, options) {
    ...
    _validateDesignators: function(item) {
        ...
        if (parseInt(qtyContent) === desigContent.length) {
            item.validate(ruleId);
            qty.validate(ruleId);
