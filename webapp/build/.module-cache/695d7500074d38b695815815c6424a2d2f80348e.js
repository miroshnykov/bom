...
var CompanyCollection = ExtendedCollection.extend({
    ...
    dispatchCallback: function(payload) {
        ...
        switch (action.type) {
            case ActionConstants.FETCH_COMPANY:
                (function() {
                    var token;
                    var company;
                    var options;

                    if (!this.validateAction(action, ["token"])) {
                        if (action.reject) {
                            action.reject(new Error("Invalid company token"));
                        }
                        return;
                    }

                    token = action.attributes.token;
                    company = new CompanyModel({ token: token });

                    options = {
                        url: company.urlRoot + "/" + token
                    };

                    company.fetch(options).then(function(model) {
                        this.add(model);

                        if (action.resolve) {
                            action.resolve(company);
                        }
